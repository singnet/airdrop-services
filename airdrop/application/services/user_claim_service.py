...
class UserClaimService:
    def __init__(self, airdrop_id):
        ...
        self.airdrop_class = self.airdrop.airdrop_processor
    ...
    def prepare_token_transfer_cardano_service_payload(self, claims):
        ...
        to_addresses = []
        for claim in claims:
            to_addresses.append({"address": claim.address, "amount": claim.claimable_amount})
        payload = {
            "token": token,
    ...
    def initiate_claim_for_users(self):
        ...
        for claim in claims:
            ClaimHistoryRepository().update_claim_status(claim.address, claim.airdrop_window_id, blockchain_method,
                                                         transaction_status)
        ...
        if transaction_status and len(transaction_status) > 0:
            transaction_status = AirdropClaimStatus.SUCCESS.value
        else:
        ...
        # Update claim status as CLAIM_FAILED/SUCCESS
        for claim in claims:
            ClaimHistoryRepository().update_claim_status(claim.address, claim.airdrop_window_id, blockchain_method,
